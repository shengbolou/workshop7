...
function postStatusUpdate(user, location, contents, image, callback) {
  ...
  app.post('/feeditem', validate({ body: StatusUpdateSchema }), function(req, res) {
    ...
    if (fromUser === body.userId) {
      postStatusUpdate(new ObjectID(body.userId), body.location, body.contents, body.image,function(err,newUpdate){
        if(err){
          res.send(500).send("A database error occurred: "+err);
        }
        else{
